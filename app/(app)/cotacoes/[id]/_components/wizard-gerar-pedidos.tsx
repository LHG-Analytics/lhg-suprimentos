...
export function WizardGerarPedidos({ open, onClose, cotacao, selecoes, fornecedores, matrizMap }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center pt-[15vh] px-4">
      ...
      <div className="relative w-full max-w-[620px] rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl overflow-hidden">

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800/80">
          <div>
            <h2 className="text-base font-semibold text-zinc-50">Gerar pedidos de compra</h2>
            <p className="text-[12px] text-zinc-500 mt-0.5">
              A cotação será dividida em <strong className="text-zinc-300">{grupos.length} pedido{grupos.length !== 1 ? "s" : ""}</strong>, um por fornecedor
            </p>
          </div>
        ...
        <div className="px-6 py-4 space-y-3 max-h-[50vh] overflow-y-auto">
          {grupos.map(({ fornecedor, itens, total }) => (
            <div key={fornecedor.id} className="rounded-xl border border-zinc-800/80 bg-zinc-900/40 p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="text-sm font-semibold text-zinc-100">{getFornNome(fornecedor)}</div>
                <div className="font-mono text-sm font-bold text-zinc-100">{formatBRL(total)}</div>
              </div>
              <div className="space-y-1">
                {itens.map(({ item, cell }) => (
                  <div key={item.id} className="flex items-center justify-between text-[12px]">
                    ...
                    <span className="text-zinc-500 font-mono shrink-0 ml-4">
        ...
        {/* Disclaimer */}
        <div className="px-6 py-3 bg-zinc-900/40 border-t border-zinc-800/60">
          <p className="text-[12px] text-zinc-500 leading-relaxed">
            Os pedidos seguirão para <strong className="text-zinc-400">aprovação</strong> antes de serem enviados aos fornecedores e sincronizados com o Omie.
          </p>
        </div>
