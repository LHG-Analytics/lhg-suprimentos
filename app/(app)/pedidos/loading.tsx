export default function PedidosLoading() {
  return (
    <div className="flex gap-0 h-[calc(100vh-56px)] animate-pulse">
      {/* Lista */}
      <div className="w-[340px] border-r border-zinc-800/60 flex flex-col shrink-0">
        {/* Header */}
        <div className="px-4 py-4 border-b border-zinc-800/60 space-y-3">
          ...
          <div className="flex gap-1.5">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-6 w-20 bg-zinc-800/60 rounded-full" />
            ))}
          </div>
        </div>
        {/* Items */}
        <div className="flex-1 overflow-hidden space-y-px p-2">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-[72px] bg-zinc-800/30 rounded-lg" />
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-6 py-5 border-b border-zinc-800/60 space-y-2">
